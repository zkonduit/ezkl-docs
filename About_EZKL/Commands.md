...
```bash
ezkl prove -M examples/onnx/1l_sigmoid/network.onnx --witness examples/onnx/1l_sigmoid/input.json --pk-path=pk.key --proof-path=model.proof --srs-path=17.srs --settings-path=settings.json
```
