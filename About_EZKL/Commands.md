...
> **Note:** The SRS generated by gen-srs is not meant to be used in production. For production usage, use the SRS generated by the Ethereum [proto-danksharding ceremony](https://ceremony.ethereum.org/). You can download it in raw format (not canonical format) and use it with `ezkl` [here](https://github.com/han0110/halo2-kzg-srs).
...
This produces a file called `output` in our `examples/onnx/1l_sigmoid` directory. it is the output of our model in JSON format. Note that `forward` quantizes the inputs and scales the outputs according to `scale`. You may yield different results from your prior tests. 
