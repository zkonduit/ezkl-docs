...
```bash
ezkl gen-witness -M network.ezkl -D examples/onnx/4l_relu_conv_fc/input.json 
```
...
```bash
ezkl prove -M network.ezkl --witness witness.json --pk-path=pk.key --proof-path=model.proof --srs-path=15.srs
```
...
ezkl setup -M conv.ezkl --srs-path=23.srs --vk-path=vkconv.key --pk-path=pkconv.key
```

```bash
# Relu
ezkl compile-circuit -M examples/onnx/1l_relu/network.onnx --settings-path=circuitrelu.json --compiled-circuit relu.ezkl
ezkl setup -M relu.ezkl --srs-path=23.srs --vk-path=vkrelu.key --pk-path=pkrelu.key
```
...
ezkl prove --proof-type=for-aggr --witness convwit.json -M conv.ezkl --proof-path conv.proof --srs-path=23.srs  --pk-path=pkconv.key
```

```bash
# Relu
ezkl gen-witness -D ./examples/onnx/1l_relu/input.json -M relu.ezkl -O reluwit.json
ezkl prove --proof-type=for-aggr --witness reluwit.json -M relu.ezkl --proof-path relu.proof --srs-path=23.srs  --pk-path=pkrelu.key 
```
